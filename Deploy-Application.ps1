...
Try {
	...
	[string]$appVendor = 'EMC'
	[string]$appName = 'Captiva Cloud Runtime'
	...
	If ($deploymentType -ine 'Uninstall') {
		...
		Show-InstallationWelcome -CloseApps 'Emc.Captiva.WebCaptureService, Emc.Captiva.WebToolkitHost' -CheckDiskSpace -Silent

		## Show Progress Message (with the default message)
		Show-InstallationProgress

		## <Perform Pre-Installation tasks here>
		## Uninstall EMC Captiva Toolkit 2.1
		$exitCode = Execute-MSI -Action 'Uninstall' -Path "{0902065E-09FA-48CE-96CC-7B5A18B5C992}"
    If (($exitCode.ExitCode -ne "0") -and ($mainExitCode -ne "3010")) {
		...
		## Install EMC Captiva Cloud Runtime 3.0
		$exitCode = Execute-Process -Path "$dirFiles\setup.exe" -Parameters "/S /v/qn" -WindowStyle "Hidden" -PassThru -WaitForMsiExec
		If (($exitCode.ExitCode -ne "0") -and ($mainExitCode -ne "3010")) {
			$mainExitCode = $exitCode.ExitCode
		}
	...
	ElseIf ($deploymentType -ieq 'Uninstall')
	{
		...
		## Uninstall EMC Captiva Cloud Runtime 3.0
		$exitCode = Execute-MSI -Action 'Uninstall' -Path "{D8C7F9E7-0B01-45F7-978F-46109C6873FA}"
    If (($exitCode.ExitCode -ne "0") -and ($mainExitCode -ne "3010")) {
			$mainExitCode = $exitCode.ExitCode
		}
